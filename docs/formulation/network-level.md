...
The project-level MILP (Eq 2.4–2.10) treats every candidate project as competing equally for one shared budget. In practice, transportation agencies often have additional constraints that the project-level model cannot express: separate funding streams for different facility categories, caps on per-region spending, and minimum-investment floors that protect underserved areas. The network-level extensions (Eq 2.14–2.16) add these constraints to the formulation while preserving the project-level structure intact.
...
The network-level constraints add to (do not replace) the project-level constraints. A solution that satisfies all of Eq 2.4–2.10 must additionally satisfy any active facility-type or regional constraints. The objective (Eq 2.4) is unchanged.
