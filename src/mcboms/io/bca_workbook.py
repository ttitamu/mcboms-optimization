"""Reader for the parallel TTI BCA workbook.

The MCBOMs project has two tooling tracks: a macro-enabled Excel BCA workbook
that handles data curation and per-segment safety benefit calculation, and
this Python codebase that handles network-level optimization. This module is
the import adapter between them.

Three things this module does:

1. Parse the BCA workbook into typed Python objects.
2. Reconstruct the workbook's per-segment safety benefit from raw inputs
   using ``mcboms.benefits.safety``, so the two implementations can be
   compared numerically.
3. Convert the BCA workbook's segment data into the ``SafetySegment``
   format the optimizer expects.

Workbook structure consumed (March 2026 schema)
-----------------------------------------------
Sheet 'Safety':
    M4, M6, M8 — unit crash costs for K, B (used for ABC), O.
    O39:O75 — combined CMFs from the CMF-combination tool, one per
    (segment, alternative) pair.

Sheet 'Segment Data':
    Columns K-O (24-28) — observed crash counts by KABCO severity, per
    segment, indexed by xd_id.
    Column F — AADT.  Column C — Length (mi).  Column A — Custom_TMCID.

Sheet 'BCA':
    Column X — precomputed Safety Benefit (used for reconciliation only).

Formula used by the workbook (cell X3):
...
(USDOT BCA Guidance 2024 value), uses K at $13.7M, and O at $5,500.
The expression is single-year and undiscounted — not a present-value
benefit. Report Equation 18, by contrast, uses 5-bin KABCO disaggregation
and discounts over the full analysis horizon. This module exposes both
representations so the optimizer can use either and reconcile against
the workbook output.
"""
...
def read_bca_workbook(path: str | Path) -> BCAWorkbookData:
    ...
    Returns:
        :class:`BCAWorkbookData` with parsed sheets ready for use by
...
def colleague_segments_to_safety_segments(
    ...
    Args:
        data: Parsed workbook from :func:`read_bca_workbook`.
